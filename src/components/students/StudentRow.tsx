...
const StudentRowComponent = ({ student }: { student: Student }) => (
  <TableRow className="hover:bg-muted/40 transition-colors">
    ...
    <TableCell className="text-sm max-w-[180px] truncate">{student.program}</TableCell>
    <TableCell>
      <Badge variant="outline" className={`${statusStyle[student.status]} font-medium text-xs`}>
        {student.status}
      </Badge>
    </TableCell>
    <TableCell>
    ...
    <TableCell className="text-sm text-muted-foreground whitespace-nowrap">{student.nextDeadline}</TableCell>
    <TableCell className="text-sm text-muted-foreground whitespace-nowrap">{student.lastActivity}</TableCell>
    <TableCell>
