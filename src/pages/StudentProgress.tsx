...
import { ArrowLeft, FileText, GraduationCap, PenLine, Activity } from "lucide-react";
import { getStudentProgress } from "@/data/studentProgress";

const StudentProgressPage = () => {
  const { studentId } = useParams();
  const navigate = useNavigate();
  const data = studentId ? getStudentProgress(studentId) : undefined;
    ...
    <CounselorLayout>
      ...
      <div className="flex items-center gap-3 mb-2">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">Track and manage {data.student.name}'s application journey</p>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <OverallProgressCard progress={data.student.progress} stages={data.stages} />
      </div>
          ...
          <Tabs defaultValue="documents" className="w-full">
            <div className="border-b border-border px-6 pt-4">
              <TabsList className="bg-transparent gap-1 h-auto p-0">
                <TabsTrigger value="documents" className="gap-2 data-[state=active]:bg-primary/10 data-[state=active]:text-primary rounded-lg">
                  <FileText className="h-4 w-4" /> Documents
                </TabsTrigger>
                <TabsTrigger value="universities" className="gap-2 data-[state=active]:bg-primary/10 data-[state=active]:text-primary rounded-lg">
                  <GraduationCap className="h-4 w-4" /> Universities
                </TabsTrigger>
                <TabsTrigger value="sop" className="gap-2 data-[state=active]:bg-primary/10 data-[state=active]:text-primary rounded-lg">
                  <PenLine className="h-4 w-4" /> SOP Review
                </TabsTrigger>
                <TabsTrigger value="activity" className="gap-2 data-[state=active]:bg-primary/10 data-[state=active]:text-primary rounded-lg">
